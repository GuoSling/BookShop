	...
	<view class="wrap">
		<view class="top">
			...
			<view class="item" @tap="showRegionPicker">
				...
				<input disabled type="text" placeholder-class="line" :value='info.province + info.city +  info.county'
					placeholder="请选择省市区" />
			</view>
		...
		<view class="bottom">
			<view class="default">
				...
				<u-switch active-color="red" v-model="info.is_default" @change="setDefault"></u-switch>
			</view>
		</view>
		<u-button type="text" style="background-color: #EE0014; color: white;" ripple size="default" @click='getInfo'>修改
		</u-button>
...
<script>
	export default {
		...
		onLoad(e) {
			...
			let info = JSON.parse(arr[0])
			this.info = info
			this.info.is_default == 1 ? this.info.is_default = true : this.info.is_default = false
		},
		methods: {
			showRegionPicker() {
				this.show = true;
			...
			getInfo() {
				let info = {
					...
					is_default:this.info.is_default == true ? 1 : 0, //是否默认
				}
				this.$u.api.updateAddress(this.info.id,info);
				uni.setStorageSync('addressId',this.info.id)
